...
namespace GMap.NET.WindowsForms
{
   ...
   public partial class GMapControl : UserControl, Interface
   {
      ...
      /// text on empty tiles
      /// </summary>
      public string EmptyTileText = "此区域无地图 \n-_-b.";
      ...
      /// <summary>
      /// import db file by name
      /// </summary>
      /// <returns></returns>
      public bool ImportDBbyName(string filename)
      {

          bool ok = GMaps.Instance.ImportFromGMDB(filename);
          if (ok)
          {
              MessageBox.Show("Complete!", "WebNode", MessageBoxButtons.OK, MessageBoxIcon.Information);
          }
          else
          {
              MessageBox.Show("Failed!", "WebNode", MessageBoxButtons.OK, MessageBoxIcon.Warning);
          }
      ...
      public bool ShowImportDialog()
      {
         using(FileDialog dlg = new OpenFileDialog())
         {
            ...
            dlg.DefaultExt = "gmdb";
            dlg.ValidateNames = true;
            dlg.Title = "WebNode: 添加数据库地图";
            dlg.FileName = "Data";
            dlg.InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            dlg.Filter = "WebNode DB files (*.gmdb)|*.gmdb";
            dlg.FilterIndex = 1;
            dlg.RestoreDirectory = true;
